...
const listUrl = (apiVersion: string) => `${API_BASE}/apis/${apiVersion}/workloads`;
...
const workloadList = (items: typeof SCHEDULING_WORKLOAD_DUMMY_DATA) =>
  HttpResponse.json({
    kind: 'WorkloadList',
    items,
    metadata: {},
...
export default {
  ...
  parameters: {
    msw: {
      handlers: {
        story: [
          http.get(listUrl(SERVED_VERSION), () => workloadList(SCHEDULING_WORKLOAD_DUMMY_DATA)),
...
Empty.parameters = {
  msw: {
    handlers: {
      story: [
        http.get(listUrl(SERVED_VERSION), () => workloadList([])),
        ...olderVersionsUnavailable,
      ],
    },
  },
