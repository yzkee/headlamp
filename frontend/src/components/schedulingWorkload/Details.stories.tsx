...
import { SCHEDULING_WORKLOAD_DUMMY_DATA } from './storyHelper';

const workload = SCHEDULING_WORKLOAD_DUMMY_DATA[0];
...
const detailsUrl = (apiVersion: string) =>
  `${API_BASE}/apis/${apiVersion}/namespaces/${namespace}/workloads/${name}`;
...
const collectionWatch = allVersions.map(apiVersion =>
  http.get(`${API_BASE}/apis/${apiVersion}/namespaces/${namespace}/workloads`, () =>
    HttpResponse.error()
  )
);
...
export default {
  title: 'SchedulingWorkload/Details',
  component: Details,
  argTypes: {},
  decorators: [
    Story => {
...
export const WorkloadDetails = Template.bind({});
WorkloadDetails.parameters = {
  msw: {
    handlers: {
      story: [
        http.get(detailsUrl(SERVED_VERSION), () => HttpResponse.json(workload)),
