...
/**
 * A fully-populated DaemonSet (tolerations, node selector, rolling update
 * strategy and a real container) shared by the List and Details stories.
 */
export const DAEMONSET_DUMMY: KubeObjectInterface = {
  apiVersion: 'apps/v1',
  kind: 'DaemonSet',
  metadata: {
...
/** The single fixture wrapped in a list, for the List stories. */
export const DAEMONSET_DUMMY_LIST: KubeObjectInterface[] = [DAEMONSET_DUMMY];

/**
 * The same DaemonSet without tolerations, to exercise the empty-tolerations
 * branch of the Details view's Tolerations section.
 */
export const DAEMONSET_NO_TOLERATIONS: KubeObjectInterface = {
  ...DAEMONSET_DUMMY,
  metadata: {
    ...DAEMONSET_DUMMY.metadata,
    name: 'gadget-no-tolerations',
    uid: 'def456',
  },
  spec: {
    ...DAEMONSET_DUMMY.spec,
    template: {
      ...DAEMONSET_DUMMY.spec.template,
      spec: {
        ...DAEMONSET_DUMMY.spec.template.spec,
        tolerations: [],
      },
    },
  },
